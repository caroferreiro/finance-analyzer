(function (global) {
  ...
  async function loadModel(options) {
    ...
    var bundle = await runtime.loadMockupBundle({
      ...
      preferStorage: bootConfig.loadProfile !== LOAD_PROFILE.SENSITIVE,
  ...
  function resolveCanonicalOwnerFromRawRow(row, ownerLookup) {
    ...
    var ownerKey = cardOwner ? cardOwner.toUpperCase() : "";
    var mappedByCardOwner = rawText(ownerLookup && ownerLookup.byCardOwner && ownerLookup.byCardOwner[ownerKey]);
  ...
  function mappedCategoryFromLookup(detail, categoryLookup) {
    ...
    var exactNorm = rawText(categoryLookup && categoryLookup[normalized]);
    if (exactNorm) {
      return exactNorm;
    }
    var prefixes = (categoryLookup && categoryLookup.__prefixes) || [];
    for (var i = 0; i < prefixes.length; i++) {
      if (normalized.indexOf(prefixes[i].prefix) === 0) {
        return prefixes[i].category;
      }
    }
    return "";
  }
  ...
  function getCategoryLookupFromCurrentMappings() {
    try {
      var mappings = ensureMappingsShape(getCurrentBundle() && getCurrentBundle().mappingsObj);
      var exact = (mappings && typeof mappings.categoryByDetail === "object") ? mappings.categoryByDetail : {};
      var prefixes = Array.isArray(mappings && mappings.categoryByDetailPrefix) ? mappings.categoryByDetailPrefix : [];
      var lookup = Object.create(null);
      Object.keys(exact).forEach(function (k) { lookup[k] = exact[k]; });
      lookup.__prefixes = prefixes;
      return lookup;
    } catch {
      return {};
    }
  ...
  function buildOwnerCategoryMatrixProjection(rows, categoryLookup, currency) {
    ...
    (rows || []).forEach(function (row) {
      ...
      var mappedCategory = mappedCategoryFromLookup(detail, categoryLookup);
  ...
  function renderOwnerCategoryMatrixSection(model) {
    ...
    if (metaNode) {
      ...
      var mappedCount = filtered.filter(function (row) {
        return Boolean(mappedCategoryFromLookup(row.detail, categoryLookup));
      }).length;
  ...
  function buildCategoryUncategorizedBacklog(rows, categoryLookup, latestMonth, sortKey) {
    ...
    (rows || []).forEach(function (row) {
      ...
      var mappedCategory = mappedCategoryFromLookup(detailKey, categoryLookup);
      if (mappedCategory) {
        return;
      }
